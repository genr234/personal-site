...
import { focusWindow, minimizeWindow, closeWindow, updateWindowPosition } from '../../lib/windowManager';
...
export function Window({ windowState }: Props) {
  const { id, title, color, icon, width, height, x, y, zIndex, focused } = windowState;
  ...
  return (
    <div
      ref={windowRef}
      class={[styles.window, focused && styles.windowFocused].filter(Boolean).join(' ')}
      style={{
        left: `${x}px`,
        top: `${y}px`,
        width: `${width}px`,
