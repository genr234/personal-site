...
export function DockItem({ id, icon, label, color, onClick, isActive, scale, onMouseEnter, onMouseLeave }: DockItemProps) {
  return (
    <button
      class={[styles.dockItem, isActive && styles.active].filter(Boolean).join(' ')}
      onClick={onClick}
