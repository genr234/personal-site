...
const float MASK_SIZE      = 10.0; // Mask size (in pixels)
const float MASK_BORDER    = 0.85; // Border intensity (0 to 1)

// Chromatic aberration offset in texels (0 = no aberration)
const vec2 ABERRATION_OFFSET = vec2(1.0, 0.0);

const float SCREEN_CURVATURE = 0.08;
const float SCREEN_VIGNETTE  = 0.35;

const float PULSE_INTENSITY = 0.02; // Intensity of pulsing animation
const float PULSE_WIDTH = 60.0; // Pulse width in pixels (times tau)
const float PULSE_RATE = 15.0; // Pulse animation speed
