...
export function closeWindow(id: string) {
  windowsSignal.value = windowsSignal.value.filter((window: WindowState) => window.id !== id);
  if (focusedWindowId.value === id) {
    const remaining = activeWindows.value;
    focusedWindowId.value = remaining.length ? remaining[remaining.length - 1].id : null;
  }
}
...
export function focusWindow(id: string) {
  windowsSignal.value = windowsSignal.value.map((window: WindowState) => {
    if (window.id === id) {
      return { ...window, focused: true, minimized: false, zIndex: ++nextZIndex };
    }
...
export function minimizeWindow(id: string) {
  windowsSignal.value = windowsSignal.value.map((window: WindowState) =>
    window.id === id
      ? { ...window, minimized: true, focused: false }
      : window
  );
